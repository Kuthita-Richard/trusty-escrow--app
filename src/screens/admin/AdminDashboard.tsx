...
import { Users, DollarSign, AlertTriangle, FileCheck, ArrowUpRight, TrendingUp } from "lucide-react";
import {
  dummyUsers,
  dummyTransactions,
  dummyDisputes,
...
export function AdminDashboard() {
  ...
  const pendingVerifications = dummyUsers.filter((u) => u.kycStatus === "submitted").length;
  const activeTransactions = dummyTransactions.filter((t) => !["released", "cancelled"].includes(t.status)).length;
  const openDisputes = dummyDisputes.filter((d) => d.status !== "resolved").length;
        ...
        <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-4">
          ...
          <Card>
            ...
            <CardContent>
              ...
              <p className="text-xs text-muted-foreground">{dummyTransactions.length} total transactions</p>
            </CardContent>
          </Card>
            ...
            <Card className="hover:border-primary/50 transition-colors cursor-pointer">
              <CardContent className="flex items-center gap-4 pt-6">
                ...
                <div>
                  ...
                  <p className="text-2xl font-bold">{dummyTransactions.filter((t) => t.status === "pending").length}</p>
                </div>
              </CardContent>
            </Card>
          </Link>
                ...
                <div className="space-y-4">
                  {recentDisputes.map((dispute) => (
                    <div key={dispute.id} className="space-y-2">
                      ...
                      <p className="text-xs text-muted-foreground">Opened {formatDate(dispute.createdAt)}</p>
                    </div>
                  ))}
                </div>
              )}
